...
// Asignar mesas según cantidad de personas (v2 - corregida)
function asignarMesas(personas, disponibles) {
    let restantes = personas;
    let asignacion = { grandes: 0, medianas: 0, pequenas: 0 };

    // Copia de disponibles para no modificar original
    let disponiblesTemp = {
        pequenas: disponibles.pequenas,
        medianas: disponibles.medianas,
        grandes: disponibles.grandes
    };

    // --- ESTRATEGIA: llenar de menor a mayor, pero con inteligencia ---

    // 1. Priorizar pequeñas SOLO si alcanzan justo o son parte eficiente
    while (restantes >= 2 && disponiblesTemp.pequenas > 0) {
        // Si restantes es 3, NO usar pequeña (preferimos una mediana)
        if (restantes === 3) break;

        // Si restantes es 5 y tenemos medianas, mejor guardar pequeñas para después
        if (restantes === 5 && disponiblesTemp.medianas > 0) break;

        // Si restantes es 7 y tenemos medianas, mejor usar dos medianas (4+4) que 2+2+2+?
        if (restantes === 7 && disponiblesTemp.medianas >= 2) break;

        asignacion.pequenas++;
        disponiblesTemp.pequenas--;
        restantes -= 2;
    }

    // 2. Llenar con medianas
    while (restantes >= 4 && disponiblesTemp.medianas > 0) {
        asignacion.medianas++;
        disponiblesTemp.medianas--;
        restantes -= 4;
    }

    // 3. Si quedan menos de 4 pero más de 0, intentar con una mediana (aunque sobre espacio)
    if (restantes > 0 && restantes < 4 && disponiblesTemp.medianas > 0) {
        asignacion.medianas++;
        disponiblesTemp.medianas--;
        restantes = 0;
    }

    // 4. Si aún quedan, usar pequeñas (restantes 1 o 2)
    while (restantes >= 2 && disponiblesTemp.pequenas > 0) {
        asignacion.pequenas++;
        disponiblesTemp.pequenas--;
        restantes -= 2;
    ...
    // 5. Si queda 1 persona, intentar con pequeña (aunque sobre espacio)
    if (restantes === 1 && disponiblesTemp.pequenas > 0) {
        asignacion.pequenas++;
        disponiblesTemp.pequenas--;
        restantes = 0;
    }

    // 6. Si aún quedan personas, usar grandes
    while (restantes >= 20 && disponiblesTemp.grandes > 0) {
        asignacion.grandes++;
        disponiblesTemp.grandes--;
        restantes -= 20;
    }

    if (restantes > 0 && disponiblesTemp.grandes > 0) {
        asignacion.grandes++;
        restantes = 0;
    }

    // Verificar si se pudo asignar todo
    if (restantes > 0) {
        return {
            disponible: false,
            mensaje: `No hay suficiente capacidad. Faltan ${restantes} asientos.`
        };
    }

    // Mensaje bonito
    const partes = [];
    if (asignacion.pequenas > 0) partes.push(`${asignacion.pequenas} pequeña${asignacion.pequenas > 1 ? 's' : ''}`);
    if (asignacion.medianas > 0) partes.push(`${asignacion.medianas} mediana${asignacion.medianas > 1 ? 's' : ''}`);
    if (asignacion.grandes > 0) partes.push(`${asignacion.grandes} grande${asignacion.grandes > 1 ? 's' : ''}`);

    let mensaje = '✅ Mesa';
    if (partes.length > 1) mensaje = '✅ Mesas';
    mensaje += ' ' + partes.join(' + ') + ` para ${personas} persona${personas > 1 ? 's' : ''}`;
